...
(() => {
  ...
  var q = () => {
    for (let [e, t] of ue) if (window.matchMedia(t).matches) return e;
    return "main";
  };
  var j = (e = document) => {
    var o;
    ...
    k = (e) => {
      let t = (n, s, i) => {
        let a = e[n],
          ...
          p;
        if (!s) return `[${m}]`;
        let u = c == null ? void 0 : c[s];
        N(u)
          ? (p = u)
          : (p = u(i && "instanceIndex" in i ? i.instanceIndex : void 0));
        let d = i && "caseInsensitive" in i && i.caseInsensitive ? "i" : "";
        if (!(i != null && i.operator)) return `[${m}="${p}"${d}]`;
        switch (i.operator) {
          case "prefixed":
            return `[${m}^="${p}"${d}]`;
          case "suffixed":
    ...
    K = async (
      ...
      try {
        ...
        let c = $(),
          ...
          u = c || r,
          d =
            (i = (s = p == null ? void 0 : p.getTime()) != null ? s : n) != null
              ? i
              : 1,
          f = u ? await dt(u, d) : null;
        if (!t || !f) {
  ...
  var H = async (e, t, o) => {
    ...
    let m = (u = _(a, "next")) == null ? void 0 : u.href,
      c = _(a, "items"),
      { length: p } = c;
    return (
      m && i !== p && (t.originalItemsPerPage = t.itemsPerPage = p),
  ...
  var C = async (e) => {
    ...
    Le = async (e, t, o) => {
      ...
      for (let p = i + 1; p <= t; p++)
        c[p] = (async () => {
          let u = c[p - 1],
            d = await K(`${a}${m}?${s}=${p}`, { cache: o });
          await u, d && (await H(d, e));
        })();
      await Promise.all(c);
    };
  var he = async (e) => {
    ...
    e.initPagination(),
      e.on("renderitems", () => {
        let { validItems: p, items: u, itemsPerPage: d } = e;
        if (!s && u.length === d) return c();
        t.style.display = p.length > d ? "" : "none";
      });
    let m = I(t, "click", async (p) => {
        p.preventDefault(), !i && ((i = !0), await V(e, s, n, p), (i = !1));
      }),
      c = () => {
  ...
  var Oe = async (e) => {
    ...
    let c = async (f) => {
      ...
      u = new IntersectionObserver((f) => {
        for (let { isIntersecting: l } of f)
          window[l ? "addEventListener" : "removeEventListener"]("scroll", p);
      }),
      d = () => {
    ...
    return (
      o.addEventListener("click", c), u.observe(t), await C(e), (a = !1), d
    );
  };
  var ke = tt($e(), 1);
  var Ke = async (e) => {
      ...
      e.initPagination(u), e.on("renderitems", () => Xt(e, d, s));
